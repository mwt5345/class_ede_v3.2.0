  ...
  if (pba->Omega0_scf != 0.){
    ...
    if (flag1 == _TRUE_){
      if (string_begins_with(string1,'y') || string_begins_with(string1,'Y')){
        ...
        /* EDE-edit:
              Define Theta_i=scf_parameters[pba->scf_parameters_size-2], and f=scf_parameters[pba->scf_parameters_size-5] , such that phi_i =Theta_i f . 
              Note that this needs an additional numerical factor to change phi from eV of Mpl (reduced Planck mass).*/
  ...
  if (ppm->primordial_spec_type == analytic_Pk) {
    ...
    if (ppt->has_scalars == _TRUE_) {
      ...
      if (ppt->has_ad == _TRUE_) {
        ...
        class_read_double("n_s",ppt->n_s_ICs);
...
int input_default_params(struct background *pba,
  ...
  // EDE-edit: Added scf parameters by hand
  pba->n_scf = 3;
  pba->f_scf = 3.973e26;
  pba->m_scf = 5.329e-27;
  pba->CC_scf = 1;
