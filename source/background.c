...
int background_functions(
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    pvecback[pba->index_bg_ddV_scf] = ddV_scf(pba,phi); // ddV_scf(pba,phi); //potential'' as function of phi derivative wrt conformal time
  ...
  /* EDE-edit: Compute scf fraction density. Note the factor of 1/3 in going from V-->rho. */
  if (pba->has_scf == _TRUE_) {
...
int background_find_f_and_zc(
    ...
    while ((index_tau_plus - index_tau_minus) > 1) {
        ...
        tau_minus = pba->tau_table[index_tau_minus];
        class_alloc(pvecback,pba->bg_size*sizeof(double),pba->error_message);
        class_call(background_at_tau(pba,tau_minus,long_info,inter_closeby,&index_tau_mid,pvecback),
               pba->error_message,
               pba->error_message);
        ...
        if (f > fmax){
            ...
            index_tau_minus = index_tau_minus + 1;
        }
        else{
            index_tau_minus = index_tau_minus + 1;
        }
...
double ddV_scf(
    ...
    return 4152.39*2.435e27*2.435e27*pow(scf_A,2)*scf_lambda*pow(1-cos(phi*2.435e27/scf_alpha),scf_lambda-1)*cos(phi*2.435e27/scf_alpha) + 4152.39*2.435e27*2.435e27*pow(scf_A,2)*scf_lambda*(scf_lambda-1)*pow(1-cos(phi*2.435e27/scf_alpha),scf_lambda-2)*pow(sin(phi*2.435e27/scf_alpha),2);   
}
